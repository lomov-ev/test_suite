...
function testSuite($testCases) {
	...
	foreach ($testCases as $testCase) {
		...
		// }
		// $testData = self::testEngine($testCase);

		$assertionResult = ($assert ? $greenColor : $redColor) . '<span style="color: ' . ($assert ? 'lawngreen;">pass' : 'orangered;">fail') . '</span>' . $resetColor;
		$testName = $testCase['testName'] ?? '';
		$output = "<code>{$testName}: {$testSubject} (<b style='color: darkseagreen;'>{$args}</b>) - {$assertionResult}</code>" . PHP_EOL;
		if ($useHtml) {
			$output = nl2br($output);
		} else {
